...
pub async fn start_server(pool: sqlx::SqlitePool, net: Arc<network::NetworkClient>, state: Arc<AppState>) {
    ...
    let status = warp::path("status").and(warp::get()).and(pf.clone()).and(nf.clone()).and(sf.clone()).and_then(handle_status);
    let trade = warp::path("trade").and(warp::post()).and(warp::body::json()).and(pf.clone()).and(nf.clone()).and_then(handle_trade);
    let withdraw = warp::path("withdraw").and(warp::post()).and(warp::body::json()).and(pf.clone()).and(nf.clone()).and_then(handle_withdraw);

    let cors = warp::cors().allow_origin("https://god-sniper-pro.netlify.app").allow_methods(vec!["GET", "POST"]).allow_headers(vec!["content-type"]);
...
async fn handle_status(pool: sqlx::SqlitePool, net: Arc<network::NetworkClient>, state: Arc<AppState>) -> Result<impl warp::Reply, warp::Rejection> {
    let user_id = "admin";
    let pubkey_str = wallet_manager::create_user_wallet(&pool, user_id).await.unwrap_or_default();
    ...
    let active_trades = match db::get_open_trades(&pool).await { Ok(t) => t.len(), Err(_) => 0 };
...
async fn handle_trade(req: TradeRequest, pool: sqlx::SqlitePool, net: Arc<network::NetworkClient>) -> Result<impl warp::Reply, warp::Rejection> {
    let user_id = "admin";
    ...
    let payer = match wallet_manager::get_decrypted_wallet(&pool, user_id).await {
        Ok(k) => k, Err(_) => return Ok(warp::reply::json(&ApiResponse { success: false, message: "Wallet Error".into(), tx_signature: "".into() }))
    };
    ...
    if req.action == "BUY" {
        if bal < (amount_lamports + 5000) {
            return Ok(warp::reply::json(&ApiResponse { success: false, message: "Fondi Insufficienti".into(), tx_signature: "".into() }));
        }
        ...
        match jupiter::get_jupiter_swap_tx(&payer.pubkey().to_string(), input, &req.token, amount_lamports, 100).await {
            Ok(mut tx) => {
                ...
                if let Ok(sig) = net.rpc.send_transaction(&tx).await {
                    ...
                    return Ok(warp::reply::json(&ApiResponse { success: true, message: "Buy Eseguito (Jupiter)".into(), tx_signature: sig.to_string() }));
                }
            },
            Err(_) => {
                // RAYDIUM FALLBACK
                if let Ok(mint) = Pubkey::from_str(&req.token) {
                     if let Ok(keys) = raydium::fetch_pool_keys_by_mint(&net, &mint).await {
                         if let Ok(sig) = raydium::execute_swap(&net, &payer, &keys, mint, amount_lamports, 0).await {
                             ...
                             return Ok(warp::reply::json(&ApiResponse { success: true, message: "Buy Eseguito (Raydium)".into(), tx_signature: sig }));
                         }
                     }
                }
            }
        ...
        return Ok(warp::reply::json(&ApiResponse { success: false, message: "Funzione Sell Manuale in arrivo. Usa Jupiter DApp per vendere ora.".into(), tx_signature: "".into() }));
    }
    
    Ok(warp::reply::json(&ApiResponse { success: false, message: "Errore generico".into(), tx_signature: "".into() }))
}
...
async fn handle_withdraw(req: WithdrawRequest, pool: sqlx::SqlitePool, net: Arc<network::NetworkClient>) -> Result<impl warp::Reply, warp::Rejection> {
    ...
    if req.token != "SOL" {
         return Ok(warp::reply::json(&ApiResponse { success: false, message: "Per sicurezza, preleva solo SOL. Converti gli altri token prima.".into(), tx_signature: "".into() }));
    }
